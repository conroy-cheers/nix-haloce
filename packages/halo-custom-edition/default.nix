...
let
  ...
  gamescopeExecutable =
    if gamescope != null && lib.meta.availableOn stdenv.hostPlatform gamescope then
      lib.getExe gamescope
    else
      null;
...
overlayfsLib.composeWindowsLayers {
  ...
  entrypointWrapper = entrypoint: ''
    ...
    for halo_arg in "$@"; do
      if [ "$halo_next_is_vidmode" = true ]; then
        halo_width="''${halo_arg%%,*}"
        halo_rest="''${halo_arg#*,}"
        halo_height="''${halo_rest%%,*}"
        halo_refresh=""
        if [ "$halo_rest" != "$halo_height" ]; then
          halo_refresh="''${halo_rest#*,}"
        fi
        if [ -n "$halo_width" ] && [ -n "$halo_height" ]; then
          case "$halo_width$halo_height$halo_refresh" in
            *[!0-9]*)
              ;;
            *)
              halo_vidmode_width="$halo_width"
              halo_vidmode_height="$halo_height"
              halo_vidmode_refresh="$halo_refresh"
              ;;
          esac
        fi
        halo_next_is_vidmode=false
    ...
    if [ -n "$halo_vidmode_width" ] && [ -n "$halo_vidmode_height" ]; then
      ${lib.optionalString (gamescopeExecutable != null) ''
        halo_gamescope_args=(
          -w "$halo_vidmode_width"
          -h "$halo_vidmode_height"
          -W "$halo_vidmode_width"
          -H "$halo_vidmode_height"
